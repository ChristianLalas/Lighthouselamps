...
app.controller('badOrderCtrl', function($scope, $http, $uibModal, $log, $filter){
	...
	$http.get(site_url + "/Bad_Order/getItems")
	...
	$scope.onSelect = function($item, $model, $label){
		...
		$scope.supplier 	= $item.supCompany;	}

	/*add item/s to the ITEM ARRAY*/
	$scope.addItem = function(itemCode, itemName, qty, unit, proType, category, itemQty,reason){
...
app.controller('confirmationCtrl', function($scope, $http, $uibModal, $uibModalInstance, confMsg, passData) {
	...
	$scope.yes = function(){
		$http.post(site_url + "/Bad_Order/insertGivBORtnsData", {
			"accoutID" 		: passData[0],
			"itemData" 		: passData[1],
			"trackID"		: passData[2],
			"issuer"		: passData[3],
			...
			window.location.assign(site_url + "/Bad_Order");
