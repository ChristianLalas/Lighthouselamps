...
var app = angular.module('badOrder', ['ui.bootstrap', 'ngAnimate', 'ngSanitize']);

app.controller('badOrderCtrl', function($scope, $http, $uibModal, $log, $filter){
	...
	$http.get(site_url + "/Purchase/getItems")
	...
	$scope.addItem = function(itemCode, itemName, qty, unit, proType, category, itemQty,supplier){
	...
	$scope.save = function(){
		...
		if(items.length === 0){
			...
			resolve: {
				confMsg : function (){
					return "Are You Sure You Want To Save Purchase Order?"
				},
				passData : function() {
					return data = [
							currentUser,
...
app.controller('confirmationCtrl', function($scope, $http, $uibModal, $uibModalInstance, confMsg, passData) {
	...
	$scope.yes = function(){
		$http.post(site_url + "/Purchase/insertGivPORtnsData", {
			"accoutID" 		: passData[0],
			"itemData" 		: passData[1],
			"trackID"		: passData[2],
			"issuer"		: passData[3],
			...
			window.location.assign(site_url + "/Purchase");
